...
def on_channel_activity():
     driver.press_keycode(22)
     time.sleep(1)
     driver.press_keycode(66)

def on_milist_activity():
...
def on_detail_activity():
     if select_by_text(driver,'播放') or select_by_text(driver,'继续'):
          time.sleep(1)
          return True
...
def is_detail_activity():
     detail = ['.DetailsActivity',
               'com.starcor.hunan.NewDetailedPageActivity']
...
def on_player_activity():
     playing_video_wait_quit(driver,100)
     if is_detail_activity():
          driver.press_keycode(4)

method_key = {'.MainActivity' : on_main_activity,
